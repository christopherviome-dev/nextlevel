...
function ShopCard({ shop, dimmed, onHover, onOpen }) {
  ...
  return (
    <button type="button" onClick={onOpen} onMouseEnter={onHover} onFocus={onHover}
      className={
        "relative text-left bg-[#241318] border border-[#3a2028] rounded-2xl overflow-hidden transition duration-200 " +
        "hover:-translate-y-1 hover:scale-[1.04] hover:ring-2 hover:ring-marigold hover:shadow-2xl hover:z-10 " +
      ...
      <div className="aspect-[4/3] bg-[#1a1015] relative">
        {cover
          ? <img src={cover} alt="" className="w-full h-full object-cover" />
          : <div className="w-full h-full flex items-center justify-center font-display text-3xl text-[#3a2028]">{name.slice(0, 2).toUpperCase()}</div>}
      ...
      <div className="p-3">
        ...
        <div className="text-xs text-[#a88b95] truncate">{shop.category || "No category"} · {shop.area || "No area"}</div>
        <div className="h-1.5 bg-[#1a1015] rounded-full mt-2 overflow-hidden">
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-0 sm:p-6" onClick={onClose}>
      <div role="dialog" aria-modal="true" aria-label={`Review ${shop.salonName || shop.name}`}
        ...
        className="bg-[#241318] border border-[#3a2028] w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl p-5">
        <div className="flex gap-3 items-start">
          {shop.profilePhoto
            ? <img src={shop.profilePhoto} alt="" className="w-16 h-16 rounded-full object-cover border border-[#3a2028]" />
            : <div className="w-16 h-16 rounded-full bg-[#1a1015] border border-[#3a2028] flex items-center justify-center text-[#a88b95] text-xs">No photo</div>}
          <div className="flex-1 min-w-0">
            <div className="text-xl font-bold text-white">{shop.salonName || shop.name}</div>
          ...
          <button ref={closeRef} onClick={onClose} aria-label="Close" className="px-3 py-1 rounded-full border border-[#3a2028] text-[#f0dad0]">✕</button>
        </div>

        {shop.bio ? <p className="text-sm mt-4 text-[#f0dad0]">{shop.bio}</p> : <p className="text-sm mt-4 text-[#a88b95]">No description yet.</p>}
        ...
        {services.length > 0 && (
          <div className="mt-4 space-y-1">
            {services.slice(0, 6).map((s) => (
              <div key={s.id} className="text-sm flex justify-between border-b border-[#3a2028] py-1">
                <span className="text-[#f0dad0]">{s.name}{s.active === false ? " (switched off)" : ""}</span>
                <span className="text-[#a88b95]">{s.price != null ? `GH₵${s.price}` : ""}{s.duration ? ` · ${s.duration}` : ""}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {workPhotos.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4">
            {workPhotos.map((s) => <img key={s.id} src={s.photo} alt={s.name} className="w-full aspect-square object-cover rounded-lg border border-[#3a2028]" />)}
          </div>
        )}
        ...
        <div className="flex gap-2 mt-5 flex-wrap">
          ...
          <a href={`/shop/${shop._id}`} target="_blank" rel="noopener noreferrer" className="px-5 py-2 rounded-full border border-[#3a2028] font-bold text-[#f0dad0]">
            Preview as customers see it
          </a>
        </div>
        <div className="text-xs text-[#a88b95] mt-2">Not ready yet? Close this. The professional can keep improving their shop, and it stays hidden until you approve.</div>
      </div>
    </div>
  );
}
