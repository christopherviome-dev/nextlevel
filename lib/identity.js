...
// Other documents (passport, licence, residence permit): letters and digits,
// 5-20 long, ignoring spaces and dashes. Mirrors the server's check.
export function normalizeIdNumber(raw) {
  const n = String(raw || "").toUpperCase().replace(/[\s-]/g, "");
  return /^[A-Z0-9]{5,20}$/.test(n) ? n : null;
}
// Shows only the last 3 characters, e.g. ******678
export const maskId = (n) => (n ? "•".repeat(Math.max(0, n.length - 3)) + n.slice(-3) : "••••");
